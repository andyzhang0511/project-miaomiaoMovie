<template>
  <div class="movie_body" ref="movie_body">
    <Loading v-if="isLoading" />
    <Scroller v-else :handleToScroll="handleToScroll" :handleToTouchEnd="handleToTouchEnd">
      <ul>
        <!-- <li>
        <div class="pic_show">
          <img src="/images/movie_1.jpg" />
        </div>
        <div class="info_list">
          <h2>无名之辈</h2>
          <p>
            观众评
            <span class="grade">9.2</span>
          </p>
          <p>主演: 陈建斌,任素汐,潘斌龙</p>
          <p>今天55家影院放映607场</p>
        </div>
        <div class="btn_mall">购票</div>
        </li>-->

        <li class="pullDown">{{pullDownMsg}}</li>

        <li v-for="item in moveList" :key="item.id">
          <div class="pic_show" @tap="handleToDetail(item.id)">
            <img :src="item.img | setWH('128.180')" />
          </div>
          <div class="info_list">
            <h2 @tap="handleToDetail(item.id)">
              {{item.nm}}
              <img v-if="item.version" src="@/assets/maxs.png" alt />
            </h2>
            <p>
              观众评
              <span class="grade">{{item.sc}}</span>
            </p>
            <p>主演: {{item.star}}</p>
            <p>{{item.showInfo}}</p>
          </div>
          <div class="btn_mall">购票</div>
        </li>
      </ul>
    </Scroller>
  </div>
</template>

<script>
// import BScroll from "better-scroll";

export default {
  name: "Nowplaying",
  data() {
    return {
      moveList: [],
      pullDownMsg: "",
      isLoading: true,
      primaryCityId: -1
    };
  },
  activated() {

    var cityId = this.$store.state.city.id;
    if (this.primaryCityId === cityId) {
      return
    }
    this.isLoading = true
    console.log(123);
    // 当有keepalive缓存系统存在的时候，mounted不会实时更新数据
    this.axios.get("/api/movieOnInfoList?cityId=" + cityId).then(res => {
      console.log(res);
      var msg = res.data.msg;
      if (msg === "ok") {
        this.moveList = res.data.data.movieList;
        this.isLoading = !this.isLoading;
        this.primaryCityId = cityId;
        // 滚屏功能  this.$nextTick作用是确保数据全部加载完毕，然后进行滚屏功能的实现
        // this.$nextTick(() => {
        //   var scroll222 = new BScroll(this.$refs.movie_body, {
        //     tap: true,
        //     probeType: 1
        //   });

        //   scroll222.on("scroll", pos => {
        //     console.log(222);
        //     if (pos.y > 30) {   //y类型：Number作用：scroll 纵轴坐标。
        //       this.pullDownMsg = "正在更新中...";
        //     }
        //   });

        //   // scroll事件  scroll
        //   // 参数：{Object} {x, y} 滚动的实时坐标
        //   // 触发时机：滚动过程中，具体时机取决于选项中的 probeType。

        //   scroll222.on("touchEnd", pos => {
        //     console.log(333);
        // if (pos.y > 30) {
        //   this.axios.get("/api/movieOnInfoList?cityId=12").then(res => {
        //     var msg = res.data.msg;
        //     if (msg === "ok") {
        //       this.pullDownMsg = "更新成功";
        //       setTimeout(() => {
        //         this.moveList = res.data.data.movieList;
        //         this.pullDownMsg = "";
        //       }, 1000);
        //     }
        //   });
        // }
        //   });

        //   // scroll事件  touchEnd
        //   // 参数：{Object} {x, y} 位置坐标
        //   // 触发时机：鼠标/手指离开。

        // });
      }
    });
  },
  methods: {
    handleToDetail(movieId) {
      this.$router.push('/movie/detail/1/' + movieId);
      // console.log(movieId);
    },
    handleToScroll(pos) {
      if (pos.y > 30) {
        this.pullDownMsg = "正在更新中...";
      }
    },
    handleToTouchEnd(pos) {
      if (pos.y > 30) {
        this.axios.get("/api/movieOnInfoList?cityId=12").then(res => {
          var msg = res.data.msg;
          if (msg === "ok") {  
            this.pullDownMsg = "更新成功";
            setTimeout(() => {
              this.moveList = res.data.data.movieList;
              this.pullDownMsg = "";
            }, 1000);
          }
        });
      }
    }
  }
};
</script>

<style scoped>
#content .movie_body {
  flex: 1;
  overflow: auto;
}
.movie_body ul {
  margin: 0 12px;
  overflow: hidden;
}
.movie_body ul li {
  margin-top: 12px;
  display: flex;
  align-items: center;
  border-bottom: 1px #e6e6e6 solid;
  padding-bottom: 10px;
}
.movie_body .pic_show {
  width: 64px;
  height: 90px;
}
.movie_body .pic_show img {
  width: 100%;
}
.movie_body .info_list {
  margin-left: 10px;
  flex: 1;
  position: relative;
}
.movie_body .info_list h2 {
  font-size: 17px;
  line-height: 24px;
  width: 150px;
  overflow: hidden;
  white-space: nowrap;
  text-overflow: ellipsis;
}
.movie_body .info_list p {
  font-size: 13px;
  color: #666;
  line-height: 22px;
  width: 200px;
  overflow: hidden;
  white-space: nowrap;
  text-overflow: ellipsis;
}
.movie_body .info_list .grade {
  font-weight: 700;
  color: #faaf00;
  font-size: 15px;
}
.movie_body .info_list img {
  width: 50px;
  position: absolute;
  right: 10px;
  top: 5px;
}
.movie_body .btn_mall,
.movie_body .btn_pre {
  width: 47px;
  height: 27px;
  line-height: 28px;
  text-align: center;
  background-color: #f03d37;
  color: #fff;
  border-radius: 4px;
  font-size: 12px;
  cursor: pointer;
}
.movie_body .btn_pre {
  background-color: #3c9fe6;
}
.movie_body .pullDown {
  margin: 0;
  padding: 0;
  border: none;
}
</style>